...
{- IGNORE THIS FOR NOW
   ...
   TODO:
     We'd like to adapt `main` below so that it:
      - takes a function declaration/statement from STDIN
      - typechecks that it looks like:  foo(x: SampleInput): SampleOutput { ... }
      - then applies the function
     We probably want compileTypeScript() to return a list of declarations with
     their input and output types, but I couldn't figure out how to do that
     with the TypeScript API, though it is surely possible with some combo of:
      - sourceFile.statements
      -	typeChecker.typeToString()
      - typeChecker.getTypeAtLocation()
	  - typeChecker.getReturnTypeOfSignature(typeChecker.getSignatureFromDeclaration(f)) 

-- This is the (TypeScript) type that the user's code should accept.
-- In reality this will be generated by hasura dynamically, based on e.g. the
-- new field the user adds in the "Add a new action" pane in the console
--
-- We're more or less following the example here:
-- https://hasura.io/docs/latest/graphql/core/actions/create.html 
typeSampleInput =
    "type SampleInput = {\n" <>
    "  username: string,\n" <>
    "  password: string,\n" <>
    "}\n"
-- ...and similarly for the expected output of the action:
typeSampleOutput =
    "type SampleOutput = {\n" <>
    "  accessToken: string,\n" <>
    "}\n"
-- (NOTE: there's an alternative UX we could imagine though: extracting a
-- graphql field from the user's TypeScript function definitions itself)
-}
...
main = do
  ...
  say "Loading our typescript transpiler, which require()'s the typescript API JS"
  ...
  say "Success! Here's the transpiled JS:"
  T.putStrLn jsCode
  ...
  say "Loading the user's transpiled function and executing it on an input"
  ...
  putStrLn $ "This is 'Number 42' if successful -----> " <> show actionRes
...
say :: String -> IO ()
say s = putStr "*** " >> putStrLn ("\ESC[92m" <> s <> "\ESC[0m")
